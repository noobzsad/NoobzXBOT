...
//Fixed Some Bug By NoobzX & RzkyO
const {
   WAConnection,
   MessageType,
   Presence,
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6285876154630:+62 812-8187-2699\n' 
            + 'END:VCARD'
prefix = '!'
blocked = []   
limitawal = 30
...
const pacarNumber = ["6285876154630@s.whatsapp.net"]
/************************************/

       
/*********** LOAD FILE ***********/
const setiker = JSON.parse(fs.readFileSync('./cld/stik.json'))
const videonye = JSON.parse(fs.readFileSync('./cld/video.json'))
const audionye = JSON.parse(fs.readFileSync('./cld/audio.json'))
const imagenye = JSON.parse(fs.readFileSync('./cld/image.json'))
	...
	itsmeiky.on('message-new', async (iky) => {
		...
		case 'giftlimit': 
				...
				const nomerr = args[0].replace('@','')
                		...
                		if (!nomerr) return reply(`maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @6285876154630 20`)
		...
		case 'apkpure':
				...
				await limitAdd(sender)
				break
		case 'jadwalbola': // Update By RzkyO & NoobzX			
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
		case 'coronainfo': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'nulis':
				...
				ct = body.slice(6)
				reply(ind.wait())
				ct = await getBuffer(`https://api.zeks.xyz/api/nulis?text=${nul}&apikey=apivinz`)
		...
		case 'hartatata':
				...
				bh = body.slice(11)
				reply(ind.wait())
				bh = await getBuffer(`https://api.zeks.xyz/api/hartatahta?text=${har}&apikey=apivinz`)
		...
		case 'pornhub':
					...
					itsmeiky.sendMessage(from, alan, image, {quoted: iky})
					await limitAdd(sender)
				break
				case 'textlight':
					if (!isRegistered) return reply(ind.noregis())
			    	...
			    	itsmeiky.sendMessage(from, lawak, image, {quoted: iky})
			   	 await limitAdd(sender)
		  	  break
		case 'hartatata2':
				 // Update By RzkyO & NoobzX				
		...
		case 'shota':
				...
				await limitAdd(sender)
				break
				case 'tolol': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
				case 'emoji2img': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
				case 'meme': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
				...
				await limitAdd(sender)
				break
				case 'darkjoke': // Update By RzkyO & NoobzX
				 // Fix Bug By RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'aesthetic':
				...
				await limitAdd(sender)
				break
		case 'logoepep': // Update By RzkyO & NoobzX 
				gatauda = body.slice(9)
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
		case 'logoepep2': // Update By RzkyO & NoobzX 
				gatauda = body.slice(10)
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
		case 'logoepep3': // Update By RzkyO & NoobzX 
				gatauda = body.slice(10)
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
		case 'logoepep4': // Update By RzkyO & NoobzX 
				gatauda = body.slice(10)
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
		case 'logoepep5': // Update By RzkyO & NoobzX 
				gatauda = body.slice(10)
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
		case 'gamelogo': // Update By RzkyO & NoobzX 
				gatauda = body.slice(9)
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
		case 'pornlogo': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'lava':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break	
                case 'faktaunik': // Fixed By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'quotes':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'covidindo': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break		
		case 'covidglobal': // Update By NoobzX & RzkyO   
				 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				itsmeiky.updatePresence(from, Presence.composing) 
				...
				await limitAdd(sender)
					break
		case 'ceknamaff': // Update By NoobzX & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					...
					await limitAdd(sender)
					break
		case 'ceknamaml': // Update By NoobzX & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					...
					await limitAdd(sender)
					break
		case 'namaninja': // Update By NoobzX & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
		...
		case 'infocuaca':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'kurs': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'github': // Update By RzkyO & NoobzX
					tels = body.slice(8) 
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'tampar':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
			    case 'liputan6': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					itsmeiky.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'foxnews': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					itsmeiky.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'nickff': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					itsmeiky.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'newsinfo': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
				case 'film': // Update By RzkyO & NoobzX	
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					itsmeiky.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'film2': // Update By RzkyO & NoobzX	
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					itsmeiky.updatePresence(from, Presence.composing) 
		...
		case 'hilih': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'holoh': // Update By NoobzX & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'halah': // Update By NoobzX & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'huluh': // Update By NoobzX & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'heleh': // Update By NoobzX & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'hurufterbalik': // Update By NoobzX & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					...
					await limitAdd(sender)
					break
		case 'lirik': // Update By NoobzX & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					teks = body.slice(6)
		...
		case 'alay':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'alay2': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'fml':	// Fixed By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'pantun':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'cersex': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					gatauda = body.slice(8)					
		...
		case 'jadwaltv':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'katadilan': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'katabucin': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'katabucin2': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'cerpen': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'quotes': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'bacotanhacker': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'quotedoraemon': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'igstalk':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'infofilm': // Update By NoobzX & RzkyO
					 // Fix Bug By NoobzX				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'infofilm2': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ige = body.slice(10)
					...
					await limitAdd(sender)
					break
		case 'heroml':  // Update By RzkyO & NoobzX
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(`[❕] Loading`)
				...
				await limitAdd(sender)
				break
				case 'igvideo': // Update By NoobzX & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
				    ...
				    break
				case 'igpost': // Update By NoobzX & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Urlnya mana tong?')
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
				    ...
				    break
				case 'igtv': // Update By NoobzX & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Urlnya mana tong?')
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
                    ...
                    itsmeiky.sendMessage(from, buffer, video, )
				    break
				case 'tiktoknowm': // Update By RzkyO & NoobzX
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Urlnya mana tong?')
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
                    ...
                    itsmeiky.sendMessage(from, buffer, video, )
				    break
				 case 'ccgenerator': // Update By RzkyO & NoobzX
					if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                   reply(`_[❕] Loading_`)
				   ...
				   await limitAdd(sender)
				   break
				 case 'ceritahorror': // Update By RzkyO & NoobzX
					if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                   reply(`_[❕] Loading_`)
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					itsmeiky.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group NoobzXBOT BOT :\n\n\`\`\``
					no = 0
...
◪ *POWERED BY OZAN*`
		...
		case 'menuowner':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					buffer = await getBuffer(`https://i.ibb.co/0ynPnqk/logo.jpg`)
		...
		case 'lpr':
					...
					itsmeiky.sendMessage('6285876154630@s.whatsapp.net', options, text, {quoted: iky})
					reply('Masalah telah di laporkan ke owner NoobzX BOT, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		case 'request':
					...
					itsmeiky.sendMessage('6285876154630@s.whatsapp.net', options, text, {quoted: iky})
					itsmeiky.sendMessage('6285876154630@s.whatsapp.net', options, text, {quoted: iky})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER NoobzX BOT, Requests palsu atau main² tidak akan ditanggapi.')
					break
		case 'blocklist': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                                ...
                                const tagzz = {
                                                text: `@${awokwkwk.split("@s.whatsapp.net")[0]} *SEDANG AFK ${tels} JANGAN GANGGU YA*`,
		...
		case 'jadwalsholat':
				...
				media = await itsmeiky.downloadAndSaveMediaMessage(boij)
				ran = getRandom('.mp3')
		...
		case 'ocr': 
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					result = fs.readFileSync(`./cld/sticker/${namastc}.webp`)
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					setiker.push(`${svst}`)
					fs.writeFileSync(`./cld/sticker/${svst}.webp`, delb)
					fs.writeFileSync(`./cld/stik.json`, JSON.stringify(setiker))
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					delb = await itsmeiky.downloadMediaMessage(boij)
					audionye.push(`${svst}`)
					fs.writeFileSync(`./cld/audio/${svst}.mp3`, delb)
					fs.writeFileSync('./cld/audio.json', JSON.stringify(audionye))
					itsmeiky.sendMessage(from, `Sukses Menambahkan Video\nCek dengan cara ${prefix}listvn`, MessageType.text, { quoted: iky })
					break
				case 'getvn':
				if (!isRegistered) return reply(ind.noregis())
					namastc = body.slice(7)
					buffer = fs.readFileSync(`./cld/audio/${namastc}.mp3`)
				...
				if (!isPremium) return reply('Maaf kamu bukan user premium!')
					...
					boij = JSON.parse(JSON.stringify(iky).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await itsmeiky.downloadMediaMessage(boij)
					imagenye.writeFileSync(`${svst}`)
					fs.writeFileSync(`./cld/image/${svst}.jpeg`, delb)
					fs.writeFileSync('./cld/image.json', JSON.stringify(imagenye))
				...
				if (!isPremium) return reply('Maaf kamu bukan user premium!')
					...
					buffer = fs.readFileSync(`./cld/image/${namastc}.jpeg`)
				...
				if (!isPremium) return reply('Maaf kamu bukan user premium!')
					...
					delb = await itsmeiky.downloadMediaMessage(boij)
					videonye.push(`${svst}`)
					fs.writeFileSync(`./cld/video/${svst}.mp4`, delb)
					fs.writeFileSync('./cld/video.json', JSON.stringify(videonye))
				...
				if (!isPremium) return reply('Maaf kamu bukan user premium!')
					namastc = body.slice(10)
					buffer = fs.readFileSync(`./cld/video/${namastc}.mp4`)
			...
			case 's': 
				...
				await limitAdd(sender)
					if ((isMedia && !iky.message.videoMessage || isQuotedImage) && args.length == 0) {
						const boij = isQuotedImage ? JSON.parse(JSON.stringify(iky).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iky
						const media = await itsmeiky.downloadAndSaveMediaMessage(boij)
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata(namo, ator)} ${ran} -o ${ran}`, async (error) => {
									//if (error) {
											...
											// fs.unlinkSync(ran)
											// }
									itsmeiky.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: iky})
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
						...
						const boij = isQuotedVideo ? JSON.parse(JSON.stringify(iky).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iky
						const media = await itsmeiky.downloadAndSaveMediaMessage(boij)
						ran = getRandom('.webp')
						reply(ind.wait())
						await ffmpeg(`./${media}`)
		...
		case 'hargapremium':
					...
					break
		case 'buypremium':
					itsmeiky.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6285876154630',MessageType.text, { quoted: iky} )
					break
		...
		case 'bc': 
					...
					if (isMedia && !iky.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST NoobzX BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
